...
SkinnedPlayListHeader::SkinnedPlayListHeader(QWidget *parent) :
    QWidget(parent)
{
    ...
    m_menu->addSeparator();
    m_menu->addAction(QIcon::fromTheme(u"list-remove"_s), tr("Remove Column"), this, &SkinnedPlayListHeader::removeColumn);

    connect(m_model, &PlayListHeaderModel::columnAdded, this, &SkinnedPlayListHeader::onColumnAdded);
    connect(m_model, &PlayListHeaderModel::columnRemoved, this, &SkinnedPlayListHeader::onColumnRemoved);
    connect(m_model, &PlayListHeaderModel::columnMoved, this, &SkinnedPlayListHeader::updateColumns);
    connect(m_model, &PlayListHeaderModel::columnChanged, this, &SkinnedPlayListHeader::updateColumns);
    readSettings();
}

SkinnedPlayListHeader::~SkinnedPlayListHeader()
{
    delete m_metrics;
    m_metrics = nullptr;
    writeSettings();
...
void SkinnedPlayListHeader::readSettings()
{
    ...
    QFont pl_font;
    pl_font.fromString(settings.value("pl_font"_L1, qApp->font().toString()).toString());
    m_pl_padding = QFontMetrics(pl_font).horizontalAdvance(u"9"_s) / 2;

    if(!m_model->isSettingsLoaded())
    {
        m_model->restoreSettings(&settings);
        QList<QVariant> sizes = settings.value("pl_column_sizes"_L1).toList();
        QList<QVariant> alignment = settings.value("pl_column_alignment"_L1).toList();
        int autoResizeColumn = settings.value("pl_autoresize_column"_L1, -1).toInt();
        int trackStateColumn = settings.value("pl_track_state_column"_L1, -1).toInt();
...
void SkinnedPlayListHeader::setNumberWidth(int width)
{
...
void SkinnedPlayListHeader::updateColumns()
{
...
int SkinnedPlayListHeader::requiredHeight() const
{
    return m_metrics->lineSpacing() + 1;
}

QList<int> SkinnedPlayListHeader::sizes() const
{
    QList<int> sizeList;
    for(int i = 0; i < m_model->count(); ++i)
...
QList<int> SkinnedPlayListHeader::alignment() const
{
...
int SkinnedPlayListHeader::trackStateColumn() const
{
    for(int i = 0; i < m_model->count(); ++i)
    {
...
int SkinnedPlayListHeader::maxScrollValue() const
{
    if(m_model->count() == 1)
        return 0;
...
int SkinnedPlayListHeader::offset() const
{
    return m_offset;
}

bool SkinnedPlayListHeader::hasAutoResizeColumn() const
{
    return m_auto_resize;
}

void SkinnedPlayListHeader::scroll(int offset)
{
    m_offset = offset;
    update();
}

void SkinnedPlayListHeader::showSortIndicator(int column, bool reverted)
{
...
void SkinnedPlayListHeader::hideSortIndicator()
{
...
void SkinnedPlayListHeader::addColumn()
{
...
void SkinnedPlayListHeader::editColumn()
{
...
void SkinnedPlayListHeader::removeColumn()
{
    if(m_pressed_column < 0)
         return;
...
void SkinnedPlayListHeader::setAutoResize(bool on)
{
   if(m_pressed_column < 0)
        return;
...
void SkinnedPlayListHeader::showTrackState(bool on)
{
    if(m_pressed_column < 0)
         return;
...
void SkinnedPlayListHeader::setAlignment(QAction *action)
{
    if(m_pressed_column < 0)
         return;
...
void SkinnedPlayListHeader::onColumnAdded(int index)
{
...
void SkinnedPlayListHeader::onColumnRemoved()
{
...
void SkinnedPlayListHeader::mousePressEvent(QMouseEvent *e)
{
    bool rtl = layoutDirection() == Qt::RightToLeft;
...
void SkinnedPlayListHeader::mouseReleaseEvent(QMouseEvent *)
{
    if(m_task == SORT)
...
void SkinnedPlayListHeader::mouseMoveEvent(QMouseEvent *e)
{
    bool rtl = layoutDirection() == Qt::RightToLeft;
...
void SkinnedPlayListHeader::resizeEvent(QResizeEvent *e)
{
    if(m_model->count() == 1)
    {
        updateColumns();
...
void SkinnedPlayListHeader::contextMenuEvent(QContextMenuEvent *e)
{
    m_pressed_pos = e->pos();
    m_pressed_column = findColumn(e->pos());
    if(m_pressed_column >= 0)
...
void SkinnedPlayListHeader::paintEvent(QPaintEvent *)
{
    bool rtl = (layoutDirection() == Qt::RightToLeft);
...
void SkinnedPlayListHeader::adjustColumn(int index)
{
    int w = 0;
...
int SkinnedPlayListHeader::autoResizeColumn() const
{
    int index = -1;
    for(int i = 0; i < m_model->count(); ++i)
    {
...
int SkinnedPlayListHeader::findColumn(QPoint pos)
{
    pos.rx() += m_offset;
    for(int i = 0; i < m_model->count(); ++i)
    {
...
int SkinnedPlayListHeader::size(int index) const
{
    return m_model->data(index, SIZE).toInt();
}

void SkinnedPlayListHeader::setSize(int index, int size)
{
    m_model->setData(index, SIZE, size);
}

const QString SkinnedPlayListHeader::name(int index) const
{
...
void SkinnedPlayListHeader::writeSettings()
{
    ...
    settings.setValue("pl_column_sizes"_L1, sizes);
    settings.setValue("pl_column_alignment"_L1, alignment);
    settings.setValue("pl_autoresize_column"_L1, autoResizeColumn);
    settings.setValue("pl_track_state_column"_L1, trackStateColumn);
    settings.endGroup();
}
