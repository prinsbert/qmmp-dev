...
#include "normalcontainer2_p.h"

NormalContainer2::NormalContainer2()
{}
...
void NormalContainer2::addTrack(PlayListTrack *track)
{
    addTracks({ track });
}

void NormalContainer2::addTracks(const QList<PlayListTrack *> &tracks)
{
...
int NormalContainer2::insertTrack(int index, PlayListTrack *track)
{
    track->m_queued_index = -1;
...
void NormalContainer2::replaceTracks(const QList<PlayListTrack *> &tracks)
{
    clearQueue();
    m_tracks.clear();
    addTracks(tracks);
...
QList<PlayListGroup *> NormalContainer2::groups() const
{
    return QList<PlayListGroup *>();
}

QList<PlayListTrack *> NormalContainer2::tracks() const
{
    return m_tracks;
}

int NormalContainer2::trackCount() const
{
    return m_tracks.count();
}

int NormalContainer2::groupCount() const
{
    return 0;
}

QList<PlayListTrack *> NormalContainer2::mid(int pos, int count) const
{
    return m_tracks.mid(pos, count);
}

bool NormalContainer2::isEmpty() const
{
    return m_tracks.isEmpty();
}

bool NormalContainer2::isSelected(int index) const
{
...
void NormalContainer2::setSelected(int index, bool selected)
{
    if (0 <= index && index < m_tracks.count())
        m_tracks.at(index)->setSelected(selected);
}

void NormalContainer2::clearSelection()
{
...
int NormalContainer2::indexOf(PlayListItem *item) const
{
    return m_tracks.indexOf(item);
}

PlayListTrack *NormalContainer2::track(int index) const
{
   if (0 <= index && index < m_tracks.count())
...
PlayListGroup *NormalContainer2::group(int index) const
{
    Q_UNUSED(index);
    return nullptr;
}

bool NormalContainer2::contains(PlayListItem *item) const
{
    return m_tracks.contains(item);
}

void NormalContainer2::removeTrack(PlayListTrack *track)
{
    removeTracks(QList<PlayListTrack *> () << track);
}

void NormalContainer2::removeTracks(QList<PlayListTrack *> tracks)
{
...
bool NormalContainer2::move(const QList<int> &indexes, int from, int to)
{
    if (from > to)
    {
...
QList<PlayListTrack *> NormalContainer2::takeAllTracks()
{
    clearQueue();
...
void NormalContainer2::reverseList()
{
...
void NormalContainer2::randomizeList()
{
