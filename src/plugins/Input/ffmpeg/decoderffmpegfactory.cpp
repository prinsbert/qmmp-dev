...
QList<TrackInfo *> DecoderFFmpegFactory::createPlayList(const QString &path, TrackInfo::Parts parts, QStringList *)
{
    ...
    if(parts & TrackInfo::MetaData)
    {
        ...
        AVDictionaryEntry *album_artist = av_dict_get(in->metadata,"album_artist",0,0);
        AVDictionaryEntry *artist = av_dict_get(in->metadata,"artist",0,0);
        AVDictionaryEntry *composer = av_dict_get(in->metadata,"composer",0,0);
        ...
        AVDictionaryEntry *year = av_dict_get(in->metadata,"date",0,0);
        ...
        if(!year)
            year = av_dict_get(in->metadata,"WM/Year",0,0);
        if(!year)
