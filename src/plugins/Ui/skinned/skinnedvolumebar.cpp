...
void SkinnedVolumeBar::mouseMoveEvent (QMouseEvent *e)
{
    if(m_moving)
    {
        int po = e->position().x();
...
void SkinnedVolumeBar::mouseReleaseEvent(QMouseEvent*)
{
    m_moving = false;
    draw(false);
    m_old = m_value;
...
void SkinnedVolumeBar::setValue(int v)
{
    if (m_moving || m_max == 0)
        return;
    m_value = v;
...
void SkinnedVolumeBar::setMax(int max)
{
    m_max = max;
    draw(false);
}
